...
- **HTML 渲染**：以 `assets/template.html` 为参考模板，按「时间线主轴 → 摘要 → 明日计划 →(hr)→ 各项目推进副轴 → 跨项目观察」的顺序填充——标题 `<h1>/<h2>/<h3>`、甘特图 `<table>`（单元格背景色着色，行=项目列=时段）、列表 `<ul><li>`（摘要里产出多的项目用嵌套 `<ul>`）、分割线 `<hr>`、加粗 `<strong>`、代码/标识符 `<code>`。全程用内联样式（字体、行高、max-width）保证邮件客户端可读。HTML 落盘为 `daily-summary-YYYY-MM-DD.html`，路径默认当前工作目录，除非用户指定别处。
