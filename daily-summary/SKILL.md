...
- **全天时间线（主轴 = 文档主体）**，由两块组成：
  - **甘特图**：用纯 `<table>` 渲染（**禁止 SVG / canvas / JS**，邮件客户端会剥离）。行 = 项目（按 digest 里 `projects[]` 顺序，主力在上），列 = 复盘窗口按小时切的 12 个时段（09、10…20，各代表该小时；窗口非 09–21 时按实际 `window` 切列）。某项目在某小时内有 `span` 覆盖或有 `user_msgs[].t` 落入，就把该格着色，否则留白；这样横向看是一个项目从几点干到几点，纵向看是同一时段在并行/切换哪些项目。每个项目一个固定色（见模板色板），图下加一行「色块 → 项目名」图例。格子里不放文字，只靠颜色，保证窄屏可读。
